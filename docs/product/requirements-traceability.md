...
| BR-09 | Kritik veri işlemleri audit log'a yazılır. | Owner audit günlüğünü maskeli aktör/request iziyle görür; diğer roller açıklayıcı yetki durumu görür | Workspace ve fırsat kritik işlemleri redakte DTO/API kullanır; yeni domain komutları aynı audit sözleşmesine bağlanır | Append-only audit/aktivite, request UUID, hassas metadata constraint'i ve owner-only RLS | [Kritik işlem, redaksiyon, RLS ve append-only testleri](../../supabase/tests/0006_history_audit_infrastructure.test.sql) | Uygulandı |
| BR-10 | Telefon normal liste ekranlarında maskelenir. | Mobil güvenlik kartı maske sözleşmesini gösterir; kişi listesi henüz yoktur | Koruma katmanı yalnız son iki haneyi taşıyan `MaskedPhoneDto` üretir; audit'li reveal komutu henüz yoktur | Şifreli PII ayrı; açık kolon grant'i yok | [Normalizasyon, maskeli DTO ve PII sızıntı testleri](../../src/server/pii/protect-phone-core.test.ts) | Planlandı |
