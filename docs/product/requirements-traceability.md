...
| BR-05 | Mükerrer kayıt kullanıcı onayı olmadan otomatik birleştirilmez. | Hızlı form otomatik birleştirme yapılmadığını açıklar; aday nedeni ve karar ekranı sonraki dilimdedir | Hızlı ekleme her varlığı ayrı oluşturur; gelecekteki birleştirme komutu açık karar ve yetki isteyecektir | Eşit ilan/telefon index'leri unique değildir ve domain komutu merge yapmaz; `duplicate_reviews` karar kaydı planlıdır | [Aynı ilan ve telefonun ayrı kaldığını kanıtlayan DB testi](../../supabase/tests/0007_quick_fsbo_command.test.sql); beş aşamalı aday testi planlı | Planlandı |
